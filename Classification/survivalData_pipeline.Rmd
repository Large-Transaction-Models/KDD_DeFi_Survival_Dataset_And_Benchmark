...
This **logistic regression model** employs **Lasso regularization (L1 penalty)** for **binary classification**, utilizing the `glmnet` package to enhance **feature selection** and **predictive accuracy**. The function begins with **data preprocessing**, where numeric features in the `train_set`, `validation_set`, and `test_data` are **standardized** (mean = 0, standard deviation = 1) to ensure stable optimization. Categorical variables are automatically handled by converting the dataset into a **matrix format** using `model.matrix()`, which excludes the intercept and prepares the data for use with `glmnet`. The logistic regression model is trained with **Lasso regularization** (`alpha = 1`), which **shrinks irrelevant feature coefficients to zero**, effectively performing **automatic feature selection** to improve model interpretability and reduce overfitting. To optimize model performance, **cross-validation (`cv.glmnet()`)** is performed to explore different values of the **regularization parameter (`lambda`)**, selecting the **optimal lambda (`lambda.min`)** that minimizes the cross-validation error. This ensures an optimal balance between **model complexity and generalization**, preventing overfitting while maintaining predictive power. After determining the **best lambda**, the model **predicts event probabilities** on both the `validation_set` and `test_data`. These probabilities are converted into **binary class labels ("yes" or "no")** using a specified **decision threshold (default = 0.5)**, which can be adjusted to account for class imbalance. The model’s classification performance is assessed using **confusion matrices**, and key metrics are computed through the `calculate_model_metrics()` function.
...
This **decision tree model** is a **binary classification algorithm** that leverages the `rpart` package to construct an optimized **CART (Classification and Regression Trees) model**. It begins with **data partitioning**, splitting the dataset into an **80% training set and a 20% validation set** to enable model evaluation. If `if_smote = TRUE`, **SMOTE (Synthetic Minority Over-sampling Technique)** is applied to the training data to address class imbalance while avoiding data leakage. The model is then trained using the **"class" method**, ensuring proper handling of categorical target variables. Key hyperparameters, including **`cp = 0.01`** (complexity parameter for pruning), **`maxdepth = 30`** (maximum tree depth to balance complexity and generalization), and **`minsplit = 20`** (minimum number of observations required to split a node), are set through `rpart.control()` to optimize model performance. After training, the model makes **predictions on the validation set**, and its performance is evaluated using a **confusion matrix** and key classification metrics via `calculate_model_metrics()`. The trained decision tree is then applied to the **test dataset**, where predicted labels are compared against actual values, and a confusion matrix is generated to assess performance.
...
This **Naive Bayes model** is a **probabilistic classifier** designed for **binary classification**, utilizing the `e1071` package to implement **Bayes' theorem** under the assumption of **feature independence**. The function begins with **data preprocessing**, ensuring that the target variable (`event`) is explicitly converted into a **factor** to maintain compatibility with the Naive Bayes algorithm. The dataset is then **split into an 80% training set and a 20% validation set**, enabling model evaluation and performance tuning. To prepare for classification, **feature matrices** for the **validation and test sets** are created by removing the target column, ensuring that the classifier only relies on predictor variables. The **Naive Bayes model** is trained using the `naiveBayes()` function, which estimates **prior probabilities** for each class and calculates **conditional probabilities** for each feature based on the training data distribution. Predictions are then generated for the **validation and test datasets**, providing both **class labels** and **probability scores**. To accommodate potential **class imbalance**, the model applies a **decision threshold (default = 0.5)** when converting predicted probabilities into binary classifications. The function then constructs **confusion matrices** for both validation and test sets, ensuring consistency by converting predictions and actual labels into factors with the same levels. Model performance is assessed using key **classification metrics** are computed through the `calculate_model_metrics()` function.
...
This **XGBoost model** is a high-performance **gradient boosting framework** tailored for **binary classification**, leveraging **parallel computation**, **tree-based learning**, and **regularization techniques** to enhance predictive accuracy. The function begins with **data preprocessing**, where numeric features in the training, validation, and test datasets are **standardized** to improve optimization stability. The target variable (`event`) is transformed into **binary labels (0/1)** to align with XGBoost’s **logistic regression objective** (`binary:logistic`). The dataset is then converted into **DMatrix format**, an optimized data structure that significantly boosts computational efficiency and memory usage. To handle class imbalance, the function calculates **scale_pos_weight**, which adjusts the model's sensitivity to minority class instances. If `if_smote = TRUE`, **SMOTE (Synthetic Minority Over-sampling Technique)** is applied to the training set, generating synthetic samples to mitigate class imbalance without introducing data leakage. The model is configured with **hyperparameters** such as `max_depth = 6`, `eta = 0.1`, `subsample = 0.8`, and `colsample_bytree = 0.8`, ensuring a balance between model complexity and generalization. A **learning rate (`eta`)** of 0.1 enables stable convergence, while `max_depth` limits tree expansion to prevent overfitting. The **subsample** and **colsample_bytree** parameters introduce **randomness in feature selection**, increasing model robustness against noise and reducing overfitting. The model is trained using **200 boosting rounds**, incorporating **early stopping (`early_stopping_rounds = 10`)**, which halts training if the validation loss does not improve, thus optimizing training efficiency. After training, the model generates **event probability predictions** for the validation and test datasets. These probabilities are converted into **binary class labels** using the specified **decision threshold (default = 0.5)**, which can be adjusted to accommodate class imbalance. The function then constructs **confusion matrices** to compare predicted vs. actual outcomes, ensuring predictions align with the expected factor levels. Finally, classification performance is assessed using key metrics computed through `calculate_model_metrics()`.
...
This **Elastic Net model** is a **regularized logistic regression classifier** that integrates **Lasso (L1) and Ridge (L2) penalties**, striking a balance between **feature selection** and **coefficient shrinkage**. The function begins with **data preprocessing**, where numeric features are **standardized** (mean = 0, standard deviation = 1) to enhance optimization stability, while categorical features are transformed into a **matrix format (`model.matrix()`)**, ensuring compatibility with `glmnet`. The dataset is then split into **80% training and 20% validation sets**, allowing for model evaluation and hyperparameter tuning. Elastic Net is trained using an **alpha value of 0.5**, meaning **equal weight is given to both L1 and L2 regularization**, promoting **sparse feature selection** while maintaining **feature correlations**. The function then employs **cross-validation (`cv.glmnet()`)**, which systematically explores different values of **lambda (the regularization parameter)**. The **optimal lambda (`lambda.min`)** is chosen based on the lowest cross-validation error, ensuring the model achieves a suitable balance between **bias and variance**. Once the **best lambda** is identified, the model predicts **event probabilities** for both validation and test datasets. These probabilities are converted into **binary class labels** (“yes” or “no”) using a specified **decision threshold (default = 0.5)**. Since class imbalance can affect predictions, the threshold can be adjusted for better calibration. The function then constructs a **confusion matrix** to compare predicted and actual values, followed by computing essential **classification metrics** using the `calculate_model_metrics()` function.
...
naive_bayes_op <- function(train_data, test_data, threshold = 0.5) {
  ...
  # Evaluate model performance with a confusion matrix for validation set
  validation_conf_matrix <- table(Predicted = binary_validation_predictions, 
                                  Actual = validation_set[[target_column]])
  validation_metrics <- calculate_model_metrics(validation_conf_matrix, validation_probs, 
                                                "Naive Bayes (Validation)")
...
deephit_model <- function(train_data, test_data, epochs = 10, num_nodes = c(64L, 64L), 
  ...
  # Data Validation: Ensure both training and testing datasets are provided, not empty, and contain the 
  # required 'event' column.
  ...
  # Preprocess: Remove the "id" column if it exists in any of the datasets, as it is not needed for 
  # model training.
...
transformation_surv_model <- function(train_data, test_data, epochs = 10, num_nodes = c(64L, 64L), dropout = 0, 
  ...
  # library(reticulate) # Enables integration with Python, allowing R to call Python functions
  # library(dplyr) # Provides functions for data manipulation and transformation
  # Data Validation: Ensure that training and testing datasets are provided, not empty, and include the 
  # 'event' column.
  if (is.null(train_data) || is.null(test_data)) {
    stop("Error: train_data or test_data is NULL!")
  }

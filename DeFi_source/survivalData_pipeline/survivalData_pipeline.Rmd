...
"randomForest", "caret", "survival", "survminer", "ggplot2", "kableExtra", "rpart", "glmnet", "data.table", "class", "reshape2", "pander", "readr", "dplyr", "e1071", "ROSE")
...
* Summary of work
  ...
  * Classification model: `logistic_regression`, `decision_tree`, `random_forest`, `KNN(not used)`
  , `Naive_bayes(not recommended)`
...
get_classification_cutoff <- function(indexEvent, outcomeEvent){
  ...
  data <- data.frame(
    ...
    OutcomeEvent = c("Account Liquidated", "Deposit", "Full Repay", "Liquidation Performed", 
                     "Repay", "Withdraw", "Account Liquidated", "Borrow", 
                     "Liquidation Performed", "Repay", "Withdraw", 
                     "Account Liquidated", "Borrow", "Deposit", 
                     "Liquidation Performed", "Withdraw", "Account Liquidated", "Borrow", 
                     "Deposit", "Liquidation Performed", "Repay"),
  ...
  return(data %>% filter(IndexEvent == str_to_title(indexEvent), OutcomeEvent ==
                           str_to_title(outcomeEvent)) %>% pull(ConvergedRMST_5))
}
```
...
The `random_forest` function trains a Random Forest classification model on the provided training data after preparing it by removing high-cardinality categorical columns (those with more than 53 unique levels), which could hinder model performance. The function then optimizes the model with hyperparameters, specifically tuning the number of trees (`ntree`) and the `mtry` parameter for optimal feature selection. After training, it predicts event probabilities on the testing data, generates a confusion matrix to assess prediction accuracy, and calculates relevant model performance metrics. 
...
(KNN will not be included in the latest pipeline due to its excessively high processing time for new datasets, making it unsuitable for the latest requirements. While scaling and normalization can improve its adaptability to certain datasets, it remains impractical for our current needs. As a result, we have decided to exclude KNN from consideration in the updated pipeline.)

This `KNN` function implements a K-nearest neighbor-based classification model for classifying and predicting the test data set. The function first extracts features and target variables from the training and test sets, and ensures that the feature matrix contains only numerical data. To improve model performance, the feature values are standardized (zero mean, unit variance) to avoid the impact of feature range differences on distance calculations. Subsequently, the function uses the `knn` method to classify and predict the test data, and generates a confusion matrix based on the predicted results and actual labels to calculate model performance indicators. Finally, the model's evaluation indicators are formatted as a data frame for further analysis and comparison.
...
### def Naive_bayes (@Author: Haolin Luo)

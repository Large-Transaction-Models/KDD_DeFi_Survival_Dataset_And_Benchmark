aft_regression <- function(train, test, features = NULL){
  ...
  # Check for reserves that have at least one column with only NA values
  # all that satisfy that are removed.
  train <- train %>%
    group_by(reserve) %>%
    filter(
      n() >= 10, # Keep groups with at least 10 rows
      !any(colSums(!is.na(across(everything()))) == 0) # No column has all NA values
    ) %>%
    ungroup() %>%
    mutate(reserve = factor(reserve)) # Drop unused factor levels
  
  
  # Remove the levels from the factor "reserve" that were removed in the 
  # previous step.
  train$reserve <- droplevels(train$reserve)

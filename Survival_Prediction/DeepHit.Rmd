...
required_packages <- c(
  ...
  "survivalmodels", "reticulate", "lubridate", "caret", "knitr"
)
...
```
```{r}
# Define S3 method to extract survival probabilities from a deephit model
predictSurvProb.deephit <- function(object, newdata, times, ...) {
  surv_mat <- predict(object, newdata = newdata, type = "surv", times = times)
  as.matrix(surv_mat)
}
...
load_data <- function(indexEvent = "borrow", outcomeEvent = "repay") {
  ...
  source("~/KDD_DeFi_Survival_Dataset_And_Benchmark/source/dataLoader.R")
  source("~/KDD_DeFi_Survival_Dataset_And_Benchmark/StudentData/preprocessing.R")
...
for (pair in all_pairs) {
  ...
  # Define a time grid for IBS calculation using the test data's time range
  max_time <- max(mini_test$timeDiff, na.rm = TRUE)
  time_grid <- seq(0, max_time, length.out = 100)

  # Compute the IBS using the pec package
  pec_obj <- pec(
    object = list(DeepHit = final_model$model),
    formula = Surv(timeDiff, status) ~ 1,
    data = mini_test,
    times = time_grid,
    cens.model = "marginal"
  )
  ibs_value <- ibs(pec_obj)["DeepHit"]

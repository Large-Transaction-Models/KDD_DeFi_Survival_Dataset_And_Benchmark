...
# Example cross-validation pipeline 
  ...
  set.seed(123)
  mini_train <- train %>% sample_n(min(nrow(train), 800))
  mini_test  <- test  %>% sample_n(min(nrow(test), 200))
...
# 2. Loop over each pair, load data, train DeepHit, and capture C-index
for (pair in all_pairs) {
  ...
  set.seed(123)
  mini_train <- train %>% dplyr::sample_n(min(nrow(train), 800))
  mini_test  <- test  %>% dplyr::sample_n(min(nrow(test), 800))
  ...
  final_model <- deep_surv_pipeline(
    ...
    deepsurv_model <- final_model$model  # Extract the trained DeepSurv model
    predicted_survival_probs_ds <- predict(deepsurv_model, mini_test, times = times)
    predicted_survival_probs_ds <- as.matrix(predicted_survival_probs_ds)
    brier_scores <- numeric(length(times))
    
    for (i in seq_along(times)) {
      t <- times[i]
      surv_prob_t <- predicted_survival_probs_ds[, i]  # Get predicted survival probabilities for time t

...
for(indexEvent in basicEventTypes$type){
  ...
  liquidations <- liquidations %>%
    mutate(type = "liquidation performed") %>%
    select(-user) %>%
    rename(user = liquidator)

  transactionsWithLP <- basicTransactions %>%
    bind_rows(liquidations)

  build_survival_data(transactionsWithLP,
                      indexEvent,
                      outcomeEvent,
                      subjects = c("user"),
                      train_cutoff,
...
# #####
# dir.create(paste0(dataPath, "Borrow/", "Full Repay/"))
# 
# cumulativeBorrows <- transactions %>%
#   filter(type == "borrow") %>%
#   group_by(user, reserve) %>%
#   arrange(timestamp) %>%
#   mutate(totalBorrowedNative = cumsum(amount), totalBorrowedUSD = cumsum(amountUSD)) %>%
#   ungroup()
# 
...
#   group_by(user, reserve) %>%
#   arrange(timestamp) %>%
#   mutate(totalRepaidNative = cumsum(amount), totalRepaidUSD = cumsum(amountUSD)) %>%
#   ungroup()
# 
# survivalData <- NULL
# 
...
#          indexTime = timestamp) %>%
#   dplyr::select(indexID, timestamp, indexTime, any_of(subjects), totalBorrowedNative, totalBorrowedUSD, any_of(indexCovariates)) %>%
#   data.table()
# 
...
#   dplyr::select(outcomeID, timestamp, outcomeTime, any_of(subjects), totalRepaidNative, totalRepaidUSD, any_of(outcomeCovariates)) %>%
#   data.table()
# 
...
#     mutate(`Index Event` = "borrow",
#            `Outcome Event` = "full repay")
#   
#   saveRDS(survivalData, paste0(dataPath, "Borrow/", "Full Repay/", "survivalData_101024.rds"))
